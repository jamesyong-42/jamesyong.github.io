  ...
  export default {
    ...
    methods: {
      ...
      animateElement () {
        ...
        if (this.parallaxChildren.length > 0) {
          ...
          this.parallaxChildren.forEach((d) => {
            ...
            if (offset * vh <= -startOffset) {
              ...
              switch (speedCurve) {
                case 'ease': {
                  const remappedOffset = this.easing(originMoveProgress) * length
                  console.log('!!!!!!!!!!', originMoveProgress, this.easing(originMoveProgress), length)
                  d.style.transform = `translate3d(0, ${originOffset - remappedOffset}px ,0)`
                  break
                }
                case 'linear': {
                  const remappedOffset = originMoveProgress * length
                  d.style.transform = `translate3d(${parseInt(d.dataset.xoffset) * speed}px, ${originOffset - remappedOffset}px ,0)`
                  break
                }
              }
            }
        ...
        this.el.style.transform = `translate3d(0, ${totalOffset}vh ,0)`
      },
